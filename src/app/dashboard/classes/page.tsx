    ...
    <div className="bg-background min-h-screen flex flex-col">
      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 overflow-y-auto">
          {/* Hero Banner */}
          <div className="bg-gradient-to-br from-sky-600 via-sky-500 to-cyan-500 px-8 pt-10 pb-16 text-white">
            <div className="max-w-4xl">
              <div className="text-sm uppercase tracking-[0.25em] text-white/70 mb-3">School Management</div>
              <h1 className="text-4xl font-extrabold mb-3">Classes</h1>
              <p className="text-white/80 text-lg max-w-xl">
                Select a class category to manage homework, marks, timetable, and student profiles.
              </p>
              <div className="mt-6 flex flex-wrap gap-6">
                <div className="bg-white/15 rounded-2xl px-5 py-3">
                  <div className="text-2xl font-extrabold">{totalStudents.toLocaleString()}</div>
                  <div className="text-sm text-white/70">Total Students</div>
                </div>
                <div className="bg-white/15 rounded-2xl px-5 py-3">
                  <div className="text-2xl font-extrabold">{totalClasses}</div>
                  <div className="text-sm text-white/70">Class Groups</div>
                </div>
                <div className="bg-white/15 rounded-2xl px-5 py-3">
                  ...
                  <Link key={card.slug} href={`/dashboard/classes/${card.slug}`} className="block group">
                    <div className={`h-full rounded-3xl shadow-xl overflow-hidden bg-gradient-to-br ${card.accent} text-white transition-all duration-300 group-hover:-translate-y-2 group-hover:shadow-2xl`}>
                      <div className="p-6 pb-4">
                        ...
                        <h2 className="text-2xl font-extrabold mb-1">{card.label}</h2>
                      ...
                      <div className="border-t border-white/20 grid grid-cols-3 divide-x divide-white/20">
                        {stats.map((stat) => (
                          <div key={stat.label} className="px-4 py-3 text-center">
                            ...
                            <div className="text-[11px] text-white/70 uppercase tracking-wide">{stat.label}</div>
                          </div>
                        ))}
                      </div>
                      <div className="px-6 pb-5 pt-3">
                        <div className="flex items-center gap-2 text-sm font-semibold text-white/90 group-hover:text-white transition">
          ...
          <div className="px-8 py-10">
            <div className="bg-white rounded-3xl shadow-lg border border-gray-100 p-6">
              <h2 className="text-lg font-bold text-primary mb-4">All Class Groups at a Glance</h2>
              <div className="overflow-x-auto">
                <table className="min-w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50 rounded-xl">
                      <th className="text-left px-4 py-3 rounded-l-xl font-semibold text-gray-500">Category</th>
                      <th className="text-left px-4 py-3 font-semibold text-gray-500">Class</th>
                      <th className="text-left px-4 py-3 font-semibold text-gray-500">Teacher</th>
                      <th className="text-right px-4 py-3 font-semibold text-gray-500">Students</th>
                      <th className="text-right px-4 py-3 rounded-r-xl font-semibold text-gray-500">Avg Marks</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50">
                    {Object.entries(classGroupsByCategory).flatMap(([cat, groups]) =>
                      groups.map((group, index) => (
                        <tr key={group.slug} className="hover:bg-sky-50/40 transition">
                          {index === 0 && (
                            <td className="px-4 py-3 font-semibold text-gray-500 capitalize align-top" rowSpan={groups.length}>
                              {cat}
                            </td>
                          )}
                          <td className="px-4 py-3">
                            <Link href={`/dashboard/classes/${cat}/${group.slug}`} className="font-semibold text-sky-700 hover:underline">
                              {group.label}
                            </Link>
                          </td>
                          <td className="px-4 py-3 text-gray-600">{group.teacher}</td>
                          <td className="px-4 py-3 text-right font-medium text-gray-900">{group.studentCount}</td>
